...
4. `SUPABASE_VERIFY.sql` 실행
5. `SUPABASE_CAFE_SEED.sql`로 카페 24개 입력
6. `TEST_ACCOUNTS_TEMPLATE.csv` 기준 테스트 계정 15개 준비
7. 테스트 계정 준비 후 리뷰 36개 입력
8. Email / Google / Apple Auth Provider 설정 확인
...
3. `SUPABASE_VERIFY.sql` 실행해서 테이블 / 컬럼 / 정책 확인
4. `SUPABASE_CAFE_SEED.sql` 실행
5. `TEST_ACCOUNTS_TEMPLATE.csv` 기준 테스트 계정 준비
6. `SUPABASE_REVIEW_SEED.sql` 또는 `REVIEW_SEED_TEMPLATE.csv`로 리뷰 입력
7. `SUPABASE_VERIFY.sql`로 카페 24개 / 리뷰 36개 / legacy review 여부 확인
8. iOS 앱에서 카페 조회 / 리뷰 / 북마크 흐름 확인
