...
- [ ] 현재 위치 기능 반영 기준으로 개인정보처리방침 / App Privacy / 공개 정책 문구 최종 재검토
- [ ] 앱스토어 제출용 설명/스크린샷 준비
- [ ] 테스트 계정 준비
현재 상태: Auth 계정 / `public.users` 정합성 / 카페 24개 / 리뷰 36개 반영과 앱 내 end-to-end QA는 확인 완료했고, 현재 앱은 이메일 로그인 + 현재 위치 기반 지도/거리 표시 1차 버전, 프로필 편집, 위치 권한/새로고침 상태 표시까지 반영했다. 지원/개인정보처리방침/이용약관 페이지는 GitHub Pages에 공개했다.
