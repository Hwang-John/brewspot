...
- v 최근 활동 기록 섹션 추가
- v 현재 위치 권한 기반 지도 이동 / 거리 표시 / 내 주변 카페 안내 추가
- v 위치 권한 안내 문구(`NSLocationWhenInUseUsageDescription`)와 권한 요청 흐름 보강
- v 홈에 현재 위치 권한/새로고침 상태 카드 추가
- v 프로필 편집 시트와 취향 입력(선호 지역 / 취향 태그 / 짧은 메모) 추가
- v 닉네임 서버 반영 + 취향 정보 로컬 저장 구조 추가
...
- v 브라우저용 UI 목업 파일 추가
- v `web/` 웹 버전 초기 프로젝트 생성 (`React + Vite + Supabase`, 데모 모드 포함)
- v 웹에 현재 위치 권한 카드 / 거리 표시 / 지도형 탐색 보드 1차 반영
- v 웹을 모바일 퍼스트 구조로 재구성 (`로그인 -> 홈 -> 상세 -> 저장/마이`, 하단 탭 이동)
- v 웹에 `커뮤니티 / 랭킹 / 홈바리스타` 별도 페이지 추가
- v 커뮤니티 / 홈바리스타 상세 읽기 페이지 추가
- v 라운지 전환 버튼과 메인 탭을 모두 하단 고정 영역으로 정리
- v 홈 첫 화면에서 라운지 세부 버튼을 제거하고 `라운지` 탭 진입 시에만 세부 탭 노출
- v 웹 하단 네비게이션을 스크롤 중에도 계속 보이는 뷰포트 고정 방식으로 조정
- v 마이페이지에 고객지원 / 개인정보·보안 / 이용약관 안내 링크 추가
- v 웹 로그인 / 홈 / 마이페이지에서 데모 노출 문구를 줄이고 정식 서비스 톤으로 정리
- v 마이페이지 안내 링크를 절대 경로 기준으로 재정리하고 로드맵 섹션 제거
- v 홈 위치 요청을 고정밀 기준으로 조정하고 실제 지도 타일 기반 지도로 교체
- v 홈 필터를 단순화하고 위치는 `watchPosition` 기반 실시간 추적으로 보강
- v 실시간 지도에 `내 위치로 이동` 버튼과 모바일 확대/축소 제스처 설정 보강
- v 로그인/운영/시드/Supabase 검증 문서 정리
- v 개인정보처리방침 / 이용약관 / 위치정보 처리 기준안 정리
- v 커뮤니티 게시판 MVP 탭 / 목록 / 상세 / 글쓰기 흐름 구현
- v 커뮤니티 게시판 Supabase 연동 + 로컬 fallback 구조 구현
- v 카페 랭킹 탭과 온디바이스 랭킹 계산 흐름 구현
- v 홈바리스타 탭 / 목록 / 상세 / 글쓰기 흐름 구현
- v 홈바리스타 Supabase 연동 + 로컬 fallback 구조 구현
- v 커뮤니티 / 랭킹 / 홈바리스타 확장 기준으로 `README` / `ONE_PAGER` / 스키마 문서 갱신
...
- v 최신 시뮬레이터 빌드 재설치 후 `커뮤니티` / `랭킹` / `홈바리스타` 탭 노출 확인
- [ ] 네트워크 실패, 권한 문제, 강제 빈 데이터 상태에서 에러 문구/로딩 상태 점검
- [ ] 위치 권한 허용 / 거부 / 설정 이동 흐름 점검
- [ ] 현재 위치 새로고침 성공/실패 문구와 거리 갱신 점검
...
- v 현재 위치 기반 지도/거리 표시 1차 반영에 맞춰 위치 처리 메모 / QA / 실행 순서 문서 정리
- v 현재 위치 기능 반영 기준으로 개인정보처리방침 / 공개 정책 문구 최신화
- [ ] 현재 위치 기능 반영 기준으로 App Privacy / App Store Connect 답변 최종 확인
- [ ] 앱스토어 제출용 설명/스크린샷 준비
- [ ] 테스트 계정 준비
현재 상태: Auth 계정 / `public.users` 정합성 / 카페 24개 / 리뷰 36개 반영 기준선과 앱 내 핵심 end-to-end 흐름은 확인 완료했고, 현재 앱은 이메일 로그인 + 현재 위치 기반 지도/거리 표시 1차 버전, 프로필 편집, 위치 권한/새로고침 상태 표시까지 반영했다. 지원/개인정보처리방침/이용약관 페이지는 GitHub Pages에 공개했고, 남은 문서 작업은 App Privacy / 스크린샷 / 테스트 계정 최종 확정이다.
...
- v 커뮤니티 게시판 MVP 탭 / 목록 / 상세 / 글쓰기 흐름 추가
- v 커뮤니티 게시판 Supabase 연동 + 로컬 fallback 구조 추가
- v 카페 랭킹 탭과 온디바이스 랭킹 계산 흐름 추가
- v 홈바리스타 탭 / 목록 / 상세 / 글쓰기 흐름 추가
- v 홈바리스타 Supabase 연동 + 로컬 fallback 구조 추가

나중에 미뤄도 되는 것:
- v 현재 위치 권한 기반 기능 추가
- [ ] 소셜 로그인 재도입
- v 프로필 편집 / 취향 입력 고도화
- v 운영 문서와 ERD를 확장 범위 기준으로 다시 정리
...
3. 위치 권한 허용 / 거부 / 설정 이동 / 새로고침 QA와 문구 최종 점검
4. 앱스토어 스크린샷용 더미 카피와 촬영 계정 상태 최종 정리

## 10. 미완료 항목 후 바로 진행할 확장 작업
1. v 커뮤니티 게시판
2. v 랭킹 고도화
3. v 홈바리스타 기능

현재 기준:
- v 커뮤니티 / 랭킹 / 홈바리스타 1차 MVP 기준선은 로컬 코드에 반영했다.
- v 원격 `main` 반영과 배포 완료

## 11. 웹 버전 전환 시작
- v iOS 앱 코드는 `ios/`에 그대로 유지
- v 웹 버전은 `web/` 폴더에서 별도로 개발 시작
- v 웹 1차 범위는 비로그인 둘러보기 / 로그인 / 카페 탐색 / 상세 / 리뷰 / 북마크 / 마이페이지 요약
- v 웹 1차 위치 기능은 브라우저 Geolocation 기반으로 시작
- v 웹에 커뮤니티 / 랭킹 / 홈바리스타 1차 MVP와 비로그인 하단 네비게이션 흐름 반영
- [ ] Supabase 실환경 `.env.local` 연결
- [ ] 웹용 커뮤니티 / 홈바리스타 상호작용 고도화 (`좋아요`, `댓글`, `저장`)
